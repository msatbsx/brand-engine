...
async function extractReferenceImages(pdfFilename: string) {
	...
	const tsOutputPath = join(ASSETS_DIR, 'references.ts');
	if (!force && existsSync(tsOutputPath)) {
		console.log('\n🖼  Skipping reference image extraction — references.ts already exists. Use yarn preprocess:force to re-run.');
		return;
	}
	...
	const exports: string[] = [
		'// Auto-generated by scripts/preprocess-pdfs.ts — do not edit manually.',
		'// Run `yarn preprocess:force` to regenerate from the source PDF.',
		'',
	...
	for (const [page, outputFilename] of Object.entries(REFERENCE_PAGES)) {
		...
		const exportName = outputFilename.replace(/-reference\.jpg$/, '').replace(/-/g, '_');
		const label = LABELS[outputFilename] ?? outputFilename;

		exports.push(`export const ${exportName}: ReferenceImage = {`);
		exports.push(`  label: ${JSON.stringify(label)},`);
		exports.push(`  data: '${base64}'`);
		exports.push('};');
		exports.push('');

		console.log(`  ✅ ${outputFilename} → ${exportName} (${base64.length.toLocaleString()} chars)`);
	}

	await writeFile(tsOutputPath, exports.join('\n'), 'utf-8');
	console.log(`  ✅ Saved → references.ts`);
}
